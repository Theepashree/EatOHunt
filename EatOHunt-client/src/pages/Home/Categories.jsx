...
const Categories = () => {
  return (
    <div className="section-container py-16 bg-white">
      ...
      {/*Category cards*/}
      <div className="flex flex-col sm:flex-row flex-wrap gap-8 justify-around items-center mt-12 bg-white">
        {categoryItems.map((item, i) => (
          ...
          >
            ...
            <div className="mt-5 space-y-1">
              <h5 className="text-[#4A4A4A] font-semibold">{item.title}</h5>
              <p className="text-[#4A4A4A]">{item.des}</p>
            </div>
          </div>
        ))}
      </div>
